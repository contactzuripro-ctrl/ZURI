...
| 2026-07-10 | Cartes en verre (glassmorphism) | Les box du dashboard perdent leur gris : fond translucide (`--glass-bg` à 32 %), flou d'arrière-plan `backdrop-blur(20px) saturate(170%)`, bords lumineux (bordure blanche 90 % + double reflet intérieur). Utilitaire `glass` dans `globals.css`, variables adaptées au mode nuit. Réglage validé : les bulles d'arrière-plan sont elles-mêmes floutées (`blur-3xl`) — nappes de couleur douces plutôt que formes nettes, l'effet verre reste bien visible sans que les bulles soient voyantes | `src/app/globals.css` (utilitaire `glass` + variables `--glass-*`), `src/components/ui/Card.tsx`, `OrganicBackdrop.tsx` |
