...
export function Sidebar() {
  ...
  // Largeur par breakpoint : sur desktop le menu « fermé » ne disparaît pas,
  // il se replie en rail d'icônes ; le mobile suit isMobileOpen (tiroir).
  const desktopWidth = isOpen ? "lg:w-64" : "lg:w-20";
  ...
  return (
    <>
      ...
      <div
        className={`relative mb-8 flex items-center gap-3 px-3 ${
          isOpen ? "" : "lg:justify-center lg:px-0"
        }`}
      >
        <span className="flex size-12 shrink-0 items-center justify-center rounded-[55%_45%_62%_38%/48%_60%_40%_52%] bg-accent-500 text-lg font-bold text-plum-950 shadow-md">
          Z
        </span>
        <span
          className={`text-xl font-semibold text-white ${isOpen ? "" : "lg:hidden"}`}
        >
          Zuri
        </span>
      </div>
